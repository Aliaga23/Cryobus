...
const GestionPermisoRol = () => {
  ...
  const fetchPermisos = async () => {
    try {
      const response = await axios.get('http://localhost:8081/api/permisos');
  ...
  const fetchRolPermisos = async (idRol) => {
    try {
      const response = await axios.get(`http://localhost:8081/api/permisoRol/${idRol}`);
  ...
  const handleAssignPermiso = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`http://localhost:8081/api/permisoRol/${selectedRol}`, newPermiso);
  ...
  const handleRemovePermiso = async (nro) => {
    try {
      await axios.delete(`http://localhost:8081/api/permisoRol/${selectedRol}/${nro}`);
      fetchRolPermisos(selectedRol);
    } catch (error) {
