...
const GestionTipoEnvio = () => {
  ...
  const fetchTipoEnvios = async () => {
    try {
      const response = await axios.get('http://localhost:8081/api/tipoEnvio');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editTipoEnvio) {
        await axios.put(`http://localhost:8081/api/tipoEnvio/${editTipoEnvio.ID}`, newTipoEnvio);
        setEditTipoEnvio(null);
      } else {
        await axios.post('http://localhost:8081/api/tipoEnvio', newTipoEnvio);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8081/api/tipoEnvio/${id}`);
      fetchTipoEnvios();
    } catch (error) {
